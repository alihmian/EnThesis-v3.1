...
In Chapter~\ref{chap:umf-ordering-property} we saw that if a reasonable Fra\"iss\'e order expansion $\k$ of a Fra\"iss\'e class $\k_0$ has both the Ramsey property and the ordering property, then the flow $X_\k$ is the universal minimal flow of $\Aut(\mathrm{Flim}(\k_0))$. Since the universal minimal flow is unique up to isomorphism, one expects that such expansions should themselves be unique in an appropriate sense. The aim of this chapter is to make that statement precise.
...
two Fra\"iss\'e order expansions of the same reduct class \(\k_0\).
...
\begin{definition}
    ...
    Let $\bfa=\langle \bfa_0,\prec^A\rangle$ be an $L$-structure and let
...
\begin{definition}
    ...
    For every \(\bfa=\langle \bfa_0,\prec^A\rangle\in \k\),
    ...
    \]
    then \(\langle \bfa_0,<'^{\,A}\rangle\in \k'\).
    ...
    \]
    then \(\langle \bfb_0,<^{B}\rangle\in \k\).
...
\begin{proof}
    ...
    Now assume that $\k$ has the ordering property. Let \(\bfa_0\in\k_0\).
    Choose an expansion
...
\begin{theorem}
    ...
    Let $\k$ and $\k'$ be reasonable Fra\"iss\'e order classes in the
...
\begin{proof}
    ...
    \[
        ...
        \bff'=\langle \bff_0,\prec_0'\rangle=\mathrm{Flim}(\k'),
    \]
    where the two limits are taken on the same reduct $\bff_0=\mathrm{Flim}(\k_0)$, using Chapter~\ref{chap:reasonable-expansions}.
    ...
    \[
        ...
        X_{\k'}=\overline{G_0\cdot \prec_0'}.
    \]
    By Chapter~\ref{chap:umf-ordering-property}, since both $\k$ and $\k'$ have the Ramsey property and the
    ordering property, the flows $X_\k$ and $X_{\k'}$ are both universal
    ...
    \(G_\delta\) orbit in \(X_\k\). It is dense by the definition of
    \(X_\k\). Moreover, for \(\prec\in X_\k\),
    \[
    ...
    \(\langle\bff_0,\prec\rangle\) have age \(\k\) and satisfy the
    Fra\"iss\'e extension property. Since the language and the underlying set
    are countable, these requirements are countable intersections of open
    conditions on finite restrictions of \(\prec\). Thus
    \(G_0\cdot\prec_0\) is \(G_\delta\). A compact metrizable space cannot
    contain two disjoint dense \(G_\delta\) sets, so this orbit is unique.

    The same argument applies to \(G_0\cdot\prec_0'\) in \(X_{\k'}\).
    ...
    \(\varphi(x,y)\) in $L$ defining $<'$ in $\bff$.

    By the same argument, the relation $<$ is defined in $\bff'$ by a simple
    formula \(\varphi'(x,y)\) in the language $L'$.

    Finally, let \(\bfa=\langle \bfa_0,\prec^A\rangle\in\k\).
...
\begin{proposition}
    ...
    \[
        \k=\k'.
    \]
    In particular, if \(\k=\k_0*\mathcal{LO}\),
    then $\k=\k'$.
\end{proposition}

\begin{proof}
    Assume, for example, that \(\k\subseteq \k'\).
    ...
    For the final statement, note that if \(\k=\k_0*\mathcal{LO}\),
...
Given \(\bfa=\langle \bfa_0,\prec\rangle\in\k\) and \(a,b\in A_0\) with
\(a\prec b\), we write \(\operatorname{tp}_{\bfa}(a,b)\)
...
\begin{definition}
    ...
    We say that $\k$ satisfies the \emph{triangle condition} if for any two
...
\begin{proof}
    By Theorem~\ref{thm:uniqueness-simple-bi-definability}, the classes $\k$
    and $\k'$ are simply bi-definable. In the notation of its proof, we may
    therefore realize the two Fra\"iss\'e limits on the same reduct \(\bff_0\)
    and write
    \[
...
\begin{proof}
    Let $\k'$ be any reasonable Fra\"iss\'e order expansion of $\k_0$ with
    both properties. If \(\k'\subseteq \k\),
    ...
    \[
        ...
        \k'=\k^*.
    \]
    But $\k^*=\k$ in this case, since reversing a convex ordering is again
    convex. Hence \(\k'=\k\).
\end{proof}

\begin{example}
    Let $\k_0$ be the class of all finite posets, and let $\k$ be the class of
    all finite structures \(\langle A,\sqsubset,\prec\rangle\)
...
\begin{proof}
    ...
    triangle condition: one may realize two distinct pair-types by appropriately
...
\begin{proof}
    ...
    Since $\k$ has the Ramsey property,
    ...
    On the other hand, since $\k'$ has both the Ramsey property and the ordering
    ...
    Let \(\k''=\Age(\langle \bff_0,\prec_0''\rangle)\).
    ...
    \[
        \k''\subseteq \k.
    \]
    Since \(\prec_0''\in Y\subseteq X_\k\),
